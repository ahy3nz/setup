...
def _generate_dampened_annealing_points(f, nonw_temp_low=305, nonw_temp_high=560, 
        interval=5, 
        time_anneal=25000, final_time=50000, w_temp_low=500, w_temp_high=500,
        dtemp=20,
        water_thermostat_style=None):
    ...
    for i in np.arange(1,n_points):
        time = i*interval
        # If we're beyond the annealing time, we're in the cooling period 
        # where we need to reduce the possible temps
        if time > time_anneal and (time - time_anneal)% decay_rate == 0 and len(possible_temps)>1:
            possible_temps = np.delete(possible_temps, -1)
            print(possible_temps)
        times.append(time)
